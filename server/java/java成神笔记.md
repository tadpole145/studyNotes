...
静态变量: 类加载时被初始化,所有对象通用该资源,线程不安全.如果使用public修饰,则可以直接“类名.变量名”的方式来使用。

静态方法:  属于类方法,它只能使用静态变量和调用静态方法.从Java8开始也可以有接口类型的静态方法了。

静态代码块: 在类加载的时候直接执行,静态块只在类装载入内存时，执行一次。常用于初始化类的静态变量,创建静态资源.

静态类: Java可以嵌套使用静态类，但是静态类不能用于嵌套的顶层。
...
Collection: 集合顶级接口,提供集合对象基本操作的通用接口方法.

Collections: 包装类,提供多种操作的静态方法,不可以实例化.

##### Set和List

相同点: 都继承自Collection接口,都用来存储一组相同类型的元素.

不同点:

- list: 元素放入有序, 可重复
- Set: 元素放入无序, 不可重复, 对于元素是否重复,可根据方法进行判断.

##### List的子类实现与区别

###### ArrayList

通过数组实现,可动态改变大小.线程不安全.

###### LinkedList

双链表实现, 增删性能好,查询性能弱于ArrayList

###### Vector 

线程安全的集合,Vector每次请求其大小的双倍空间，而ArrayList每次对size增长50%.性能较差.
